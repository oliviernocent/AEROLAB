...
# -*- coding: utf-8 -*-
'''
This script merges several files generated by GPS sensor GARMIN EGDE 530.
Mass concentration of each pollutant are also computed from µg/m3 values.

USAGE:

./activity_??????????.tcx directory_name

where directory_name is the path to the tcx data files.
'''
...
if len(sys.argv) == 1:
    print('\nUSAGE: ./activity_??????????.tcx. directory_name')
