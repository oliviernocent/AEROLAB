...
FigT= ggplot(data=donnees, aes(y = Temperature, x = TimeNumeric, color = Period)) +
...
DT::datatable(
  val0,
  fillO3ntainer = FALSE, options = list(pageLength = 10))
...
FigO3= ggplot(data=donnees, aes(y = O3, x = Time, color = Period)) +
   scale_colour_brewer(palette="Set1", labels= c("Jour", "Nuit")) +
   geom_point() +
   scale_x_continuous(n.breaks = 5,) +
                      #limits = c(0, 24)) +
...
P0=ggplot(donnees, aes(x = Time, y = CO, color=as.factor(Period))) +
  geom_point()+
...
P3=ggplot(donnees, aes(x = Period, y = CO, color=Period)) +
  geom_boxplot()+
# stat_summary(fun=mean, geom="point", shape=20, size=4, color="red", fill="red")+
#xlab("ERFE/Fam132b")+
xlab("Period")+
